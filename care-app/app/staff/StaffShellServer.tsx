// app/staff/StaffShellServer.tsx
import StaffShell from "./StaffShell";
import { createSupabaseServerClient } from "@/lib/supabase/server";

type MonthItem = { label: string; slug: string };

function monthLabelFromSlug(slug: string) {
  const [y, m] = slug.split("-");
  const d = new Date(Date.UTC(Number(y), Number(m) - 1, 1));
  return d.toLocaleString(undefined, { month: "short", year: "numeric" });
}

export default async function StaffShellServer({
  children,
}: {
  ...
  if (error) {
    return (
      <StaffShell months={[]} error={error.message}>
        {children}
      </StaffShell>
    );
  }
  ...
  return <StaffShell months={months}>{children}</StaffShell>;
}
